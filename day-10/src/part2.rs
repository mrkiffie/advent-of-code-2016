...
fn solve(input: &str) -> u32 {
    ...
    let mut output_0: Option<ChipId> = None;
    let mut output_1: Option<ChipId> = None;
    let mut output_2: Option<ChipId> = None;
    ...
    while let Some(mut bot) = queue.pop_front() {
        if let (Some(low), Some(high), Some(low_target), Some(high_target)) = (
            ...
            if let (Some(out_0), Some(out_1), Some(out_2)) = (output_0, output_1, output_2) {
                return u32::from(out_0.0) * u32::from(out_1.0) * u32::from(out_2.0);
            }
